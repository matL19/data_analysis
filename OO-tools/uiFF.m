classdef uiFF < handle
    %If you're reading this but want the actual GUI, use these two lines of
    %code 
    %fh=uifigure(11)
    %lh=uiFF(fh);
...
%         UITable
    ...
    properties (SetAccess = immutable)
        PROPERTIES_TO_SAVE = {'FF'}
    end
    
    methods (Access = public)
        function obj = uiFF(Parent) %UITable was Parent previously
        ...
        function updateFFDropDown(obj,dropDown)
            searchTarget = 'FF1VoigtBnd'; %a file that should exist to help us find the folder
            if exist(searchTarget,'file')
                path = dir(which(searchTarget)).folder;
                FFNames = {dir([path,filesep,'FF*Bnd.m']).name};
            end
            dropDown.Items = [{'Select One'},FFNames(:)'];
        end
        ...
        function updateFFInnerGrid(obj);
%             disp('updateFFInnerGrid');
        ...
        function DropDownValueChanged(obj, src)
            ...
            % build empty new one
            [~,FFName,~] = fileparts([filesep,src.Value]);
            
            if exist(FFName,'file')
                obj.FF = feval(FFName);
            else
                warning(['SGRLAB:fitting function ',FFName,' not found.']);
            end
            
            % update grid accordingly
            updateFFInnerGrid(obj);
        end
        ...
        function updateFF(obj)
            ...
            for ii = nparams:-1:1 %going backwards so it is preallocated
                FFParams(ii) = fitParamBnd(names{ii},obj.s.vals(ii),obj.s.lb(ii),obj.s.ub(ii),'');
            end
            obj.FF = obj.FF.setParams2(FFParams);
            if obj.s.isFree
                obj.FF.isFree = 'free';
            else
                obj.FF.isFree = 'fixed';
    ...
    methods (Access = public) %make private?
        function obj = createComponents(obj)
            ...
            obj.OuterGrid = uigridlayout(obj.Parent); %was obj.Parent
