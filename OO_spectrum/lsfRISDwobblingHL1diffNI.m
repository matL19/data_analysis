classdef lsfRISDwobblingHL1diffNI < lineshapeFunction
    
    properties
        params = struct('Delta_cm',[],'tr1',[],'T2',[]);
        g;
        c2;
        order;
        tpoints;
    ...
    methods
        ...
        function obj = lsfRISDwobblingHL1diffNI(params,str,aRFoptions) %constructor function
            if nargin == 0
                super_args = {};
            elseif nargin == 1 
        ...
        function out = makeC2(obj)
            ...
            for l = 1:4
                %Ctot{l} = 1;
                %for ii = 1:ncones
                Ctot{l}=@(p,t) exp(-(l*(l+1)./(6.*p(1).tr)).*t);
                %end
            end
        ...
        function obj = makeL_l(obj)
            ...
            for l = 1:4
                %Ctot{l} = 1;
                %for ii = 1:ncones
                Ctot{l}= @(t,p) exp(-(l*(l+1)./(6.*p.tr1)).*t); %cone 1
                %end
            end
