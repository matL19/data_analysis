function dataOut = averagePPdata(dataIn,varargin)
...
n_zp = 1;
phase_deg = 0; %these shouldn't do anything but I'm leaving them in case
apod = 'none';
filter = false;
...
while length(varargin)>=2 %using a named pair
    ...
    switch lower(arg)
        ...
        case 'filter'
            filter = val;
        otherwise
            warning(['averagePPdata: unknown option ',arg])
    end
    varargin = varargin(3:end);
end %again none of these should do anything meaningful to PP data
...
for ii = 1:length(uniquet2s)
    ...
    [m,n] = size(temp(1).data); % figure out the new size of R
% figure out your average R -- we turn elements of the temp structure into
% a 2-D matrix, and then average along the third dimension
    R = mean(reshape([temp.data],n,length(temp)),2)'; 
    ...
    figure(3434)
    plot(dataOut(ii).freq,dataOut(ii).data)
    pause(0.15)
end