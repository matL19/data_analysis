function [out] = extractMaximaPP(dataobj,options)
...
required_fields = {'range3' 'fitfcn' 'flag_plot' 'lb' 'startpoint' 'ub'};
...
for ii = 1:length(dataobj)
    ...
    R = dataobj(ii).data;
        ...
        if estimateArea
            area = estimatePeakArea(w3,R,'fitPosPeak',fitPosPeak,'flag_plot',estimate_flag_plot);
        ...
        % the fitting part
  [fitresult,gof,fitinfo] = fit(w3(:),R(:),fitfcn, 'StartPoint',startpoint,...
       'lower',lb,'upper',ub);
  ...
  if flag_plot 
           ...
           fig.Color = 'w';
           clf
           p1 = plot(w3(:),R(:),'b.',w3a,fitresult(w3a));
           fig.Children.Box = 'off';
           fig.Children.TickDir = 'out';
           title(sprintf('%i fs',dataobj(ii).t2));
