function [dataobj] = cropDataPP(data,range3)
...
dataobj = struct('data',[],'freq',[]);
for ii = 1:numel(data)
    if isempty(data(ii).data)
        continue
    end
    
    % Allow the use of empty ranges to not crop along specific dimensions
    if isempty(range3)
        range3 = [data(ii).freq(1) data(ii).freq(end)];
    end
    ind3 = data(ii).freq>=range3(1) & data(ii).freq<=range3(2);
    dataobj(ii).freq = data(ii).freq(ind3);
    dataobj(ii).data = data(ii).data(ind3);
